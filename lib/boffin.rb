...
class Boffin
  ...
  def initialize(config = {})
    @config = Boffin.config.merge(config)
  end

  def hit(namespace, object, hitspace, *uniquenesses, opts)
    opts ||= {}
    ...
    Boffin.redis.expire(daykey, @config.daily_expire_secs)
  end
  ...
  def rdb_keyspace
    "#{@config.namespace}:#{Utils.underscore(to_s)}"
  end
